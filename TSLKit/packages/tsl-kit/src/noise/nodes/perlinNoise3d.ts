...
import { fade, mod289, permute, taylorInvSqrt } from './common.js';

type NoiseVec3 = ShaderNodeObject<Node>;

export const perlinNoise3d = /*#__PURE__*/ Fn<ShaderNodeObject<Node>>(([positionInput]: [NoiseVec3]) => {
  const P = vec3(positionInput).toVar();
  ...
  const nXYZ = float(mix(nYZ.x, nYZ.y, fadeXYZ.x)).toVar();
