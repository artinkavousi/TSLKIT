 ...
 * ```
 */
import { Fn, vec3, dot } from 'three/tsl';
/**
 ...
 */
const sepiaMatrix = vec3(vec3(0.393, 0.769, 0.189), // Red channel
 ...
 * ```
 */
export const sepia = /*@__PURE__*/ Fn(([color, amount = 1.0]) => {
    // Extract RGB from input (handles vec3 or vec4)
    const c = vec3(color);
    // Apply sepia transformation
    const sepiaColor = vec3(dot(c, sepiaMatrix.x), dot(c, sepiaMatrix.y), dot(c, sepiaMatrix.z));
