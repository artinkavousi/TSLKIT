...
function determinePerformanceTier(limits) {
    ...
    if (storageSize >= 32768 && bufferSize >= 2147483648) {
        return 'ultra';
    }
    // High: Mid-to-high desktop GPUs (RTX 2070+, RX 5700+)
    if (storageSize >= 16384 && bufferSize >= 1073741824) {
        return 'high';
    }
    // Mid: Entry-level desktop / high-end mobile (GTX 1660, integrated)
    if (storageSize >= 8192 && bufferSize >= 536870912) {
        return 'mid';
    }
...
export function meetsRequirements(capabilities, requirements) {
    ...
    if (requirements.minComputeUnitsPerDispatch) {
        ...
        if (available < requirements.minComputeUnitsPerDispatch) {
            missing.push(`Insufficient compute units: ${available} < ${requirements.minComputeUnitsPerDispatch}`);
        }
    }
    ...
    if (requirements.minStorageBufferSize) {
        ...
        if (available < requirements.minStorageBufferSize) {
            missing.push(`Insufficient storage buffer: ${available} < ${requirements.minStorageBufferSize}`);
        }
    }
    // Check texture size
    if (requirements.minTextureSize) {
        ...
        if (available < requirements.minTextureSize) {
            missing.push(`Insufficient texture size: ${available} < ${requirements.minTextureSize}`);
        }
    }
 ...
 */
export function getRecommendedGPUSettings(capabilities) {
 ...
 */
export function generateReport(capabilities) {
    const lines = [];
    ...
    lines.push(`  Max Storage Buffer: ${(capabilities.limits.maxStorageBufferBindingSize / 1073741824).toFixed(2)} GB`);
