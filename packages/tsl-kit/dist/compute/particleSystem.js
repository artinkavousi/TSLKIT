 ...
 * ```
 */
export function createParticleArrays(particleCount, includeColors = true, includeSizes = false) {
    return {
 ...
 * ```
 */
export function createGridInitCompute(arrays, options) {
 ...
 * ```
 */
export function createPhysicsUpdateCompute(arrays, options = {}) {
    const { gravity = -0.00098, bounce = 0.8, friction = 0.99, floorY = 0 } = options;
    return Fn(() => {
        const position = arrays.positions.element(instanceIndex);
 ...
 * ```
 */
export function createWaveUpdateCompute(arrays, particleCount, waveParams = {
    frequencyX: 0.7,
    frequencyZ: 0.5,
    amplitudeX: 50,
    amplitudeZ: 50,
