 ...
 */
function parseColor(colorConfig) {
 ...
 */
function createWoodMaterial(config) {
    let material;
    if (config.preset) {
        material = WoodNodeMaterial.fromPreset(config.preset);
    }
    else {
 ...
 */
function createMetalMaterial(config) {
    // Metal color presets
    const metalColors = {
 ...
 */
function createFabricMaterial(config) {
    const material = new THREE.MeshPhysicalMaterial({
        name: config.name,
        side: parseSide(config.side),
        transparent: config.transparent,
 ...
 * ```
 */
export function makeMaterial(config) {
    // Validate config
    ...
    switch (validated.type) {
        ...
        default:
            throw new Error(`Unknown material type: ${validated.type}`);
    }
}
/**
...
//# sourceMappingURL=makeMaterial.js.map