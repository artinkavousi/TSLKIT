 ...
 */
import { Fn, float, pow, dot, max, sqrt, PI } from 'three/tsl';
/**
...
export const anisotropicHighlight = Fn(([N, V, T, anisotropy]) => {
    ...
    const highlight = pow(float(1).sub(TdotV.mul(TdotV)).max(0), float(1).div(anisotropy.abs().max(0.001))).mul(NdotV);
    return highlight;
});
//# sourceMappingURL=anisotropy.js.map