 ...
 */
import { Fn, float, vec3, pow, dot, max, clamp, PI } from 'three/tsl';
/**
...
export const perturbClearcoatNormal = Fn(([baseNormal, clearcoatNormalMap, strength]) => {
    ...
    // Apply strength
    const scaledPerturbation = vec3(perturbation.x.mul(strength), perturbation.y.mul(strength), perturbation.z);
    // Blend with base normal
    const clearcoatNormal = baseNormal.add(scaledPerturbation).normalize();
    return clearcoatNormal;
});
//# sourceMappingURL=clearcoat.js.map