 ...
 * ```
 */
export async function getDeviceLimits() {
    if (!navigator.gpu) {
 ...
 * ```
 */
export async function selectQualityPreset() {
    try {
        const caps = await getDeviceCapabilities();
        if (!caps.supported) {
            return 'low';
        ...
        if (limits.maxTextureDimension2D >= 16384)
            score += 3;
        else if (limits.maxTextureDimension2D >= 8192)
            score += 2;
        ...
        if (features.includes('texture-compression-bc'))
            score += 1;
        if (features.includes('texture-compression-etc2'))
            score += 1;
        if (features.includes('texture-compression-astc'))
            score += 1;
        ...
        if (vendor.toLowerCase().includes('amd'))
            score += 0.5;
        if (architecture.toLowerCase().includes('integrated'))
            score -= 1;
        // Map score to preset
        if (score >= 7)
            return 'ultra';
 ...
 * ```
 */
export function getQualitySettings(preset) {
    const settings = {
        low: {
            preset: 'low',
            shadowMapSize: 512,
