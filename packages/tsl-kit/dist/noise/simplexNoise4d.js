...
const grad4 = Fn(([j_immutable, ip_immutable]) => {
    ...
    const p = vec4().toVar();
    const s = vec4().toVar();
    p.xyz.assign(floor(fract(vec3(j).mul(ip.xyz)).mul(7.0))
        .mul(ip.z)
        .sub(1.0));
 ...
 * ```
 */
export const simplexNoise4d = Fn(([v_immutable]) => {
    ...
    const j1 = vec4(permute4d_vec4(permute4d_vec4(permute4d_vec4(permute4d_vec4(i.w.add(vec4(i1.w, i2.w, i3.w, 1.0))).add(i.z.add(vec4(i1.z, i2.z, i3.z, 1.0)))).add(i.y.add(vec4(i1.y, i2.y, i3.y, 1.0)))).add(i.x.add(vec4(i1.x, i2.x, i3.x, 1.0))))).toVar();
...
//# sourceMappingURL=simplexNoise4d.js.map