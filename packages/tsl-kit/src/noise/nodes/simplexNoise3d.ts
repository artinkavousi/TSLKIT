...
export type NoiseVec3 = ShaderNodeObject<Node>;

export const simplexNoise3d = /*#__PURE__*/ Fn<ShaderNodeObject<Node>>(([vInput]) => {
  ...
  const p = vec4(
    permute(
      permute(permute(i.z.add(vec4(0.0, i1.z, i2.z, 1.0))).add(i.y).add(vec4(0.0, i1.y, i2.y, 1.0)))
        .add(i.x)
  ...
  const h = vec4(sub(1.0, x.abs()).sub(y.abs())).toVar();
