...
import { TempNode, Vector2, NodeUpdateType } from 'three/webgpu'
...
export class DotScreenNode extends TempNode {
  ...
  constructor(
    ...
    angle: Node = float(1.57),
    scale: Node = float(1.0)
  ) {
    super('vec4')
  ...
  setup() {
    ...
    const dotScreen = Fn(() => {
      ...
      const uvRotated = vec2(
        c.mul(pattern.x).sub(s.mul(pattern.y)),
        s.mul(pattern.x).add(c.mul(pattern.y))
      ).mul(scale)

      // Create dot pattern using sine waves
      const dotSize = float(1.0).sub(
        sin(uvRotated.x).mul(sin(uvRotated.y)).mul(4.0)
      )
