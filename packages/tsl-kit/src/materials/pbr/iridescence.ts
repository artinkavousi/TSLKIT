...
import { Fn, float, vec3, pow, dot, max, min, clamp, sin, cos, mix, PI } from 'three/tsl';
...
export const iridescenceFresnel = Fn(([NdotV, baseF0, iridescenceIOR, iridescenceThickness, iridescenceAmount]) => {
  ...
  const fresnel = iridescent.add(
    float(1).sub(iridescent).mul(pow(clamp(float(1).sub(NdotV), 0, 1), 5))
  );
