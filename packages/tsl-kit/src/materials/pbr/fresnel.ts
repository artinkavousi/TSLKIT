...
import { Fn, float, vec3, pow, dot, mix, clamp, max } from 'three/tsl';
...
export const fresnelSchlick = Fn(([F0, VdotH]) => {
  const fresnel = F0.add(
    float(1).sub(F0).mul(pow(clamp(float(1).sub(VdotH), 0, 1), 5))
  );
  return fresnel;
});
...
export const fresnelSchlickRoughness = Fn(([F0, VdotH, roughness]) => {
  const fresnel = F0.add(
    max(float(1).sub(roughness), F0).sub(F0).mul(pow(clamp(float(1).sub(VdotH), 0, 1), 5))
  );
  return fresnel;
});
