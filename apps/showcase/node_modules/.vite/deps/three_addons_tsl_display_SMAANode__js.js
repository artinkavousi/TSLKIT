...
// node_modules/three/examples/jsm/tsl/display/SMAANode.js
var _quadMesh = new QuadMesh();
var _size = new Vector2();
var _rendererState;
var SMAANode = class extends TempNode {
  static get type() {
    return "SMAANode";
  }
  /**
  ...
  setup(builder) {
    ...
    const SMAAEdgeDetectionVS = Fn(() => {
      const vOffset0 = vec4(uvNode.xy, uvNode.xy).add(vec4(this._invSize.xy, this._invSize.xy).mul(vec4(-1, 0, 0, -1)));
      const vOffset1 = vec4(uvNode.xy, uvNode.xy).add(vec4(this._invSize.xy, this._invSize.xy).mul(vec4(1, 0, 0, 1)));
      const vOffset2 = vec4(uvNode.xy, uvNode.xy).add(vec4(this._invSize.xy, this._invSize.xy).mul(vec4(-2, 0, 0, -2)));
    ...
    const SMAAEdgeDetectionFS = Fn(() => {
      ...
      dot(edges, vec2(1, 1)).equal(0).discard();
    ...
    const SMAAWeightsVS = Fn(() => {
      ...
      const vOffset2 = vec4(vOffset0.xz, vOffset1.yw).add(vec4(-2, 2, -2, 2).mul(vec4(this._invSize.xx, this._invSize.yy)).mul(float(SMAA_MAX_SEARCH_STEPS))).toVar();
    ...
    const SMAAWeightsFS = Fn(() => {
      ...
      const vOffset1 = varyingProperty("vec4", "vOffset1");
      const vOffset2 = varyingProperty("vec4", "vOffset2");
      const weights = vec4(0, 0, 0, 0).toVar();
      const subsampleIndices = vec4(0, 0, 0, 0).toVar();
    ...
    const SMAABlendFS = Fn(() => {
      ...
      If(dot(a, vec4(1)).lessThan(1e-5), () => {
        result.assign(this.textureNode.sample(uvNode));
      }).Else(() => {
        const offset = vec2().toVar();
        offset.x = a.a.greaterThan(a.b).select(a.a, a.b.negate());
        offset.y = a.g.greaterThan(a.r).select(a.g, a.r.negate());
...
var SMAANode_default = SMAANode;
var smaa = (node) => nodeObject(new SMAANode(convertToTexture(node)));
export {
  SMAANode_default as default,
  smaa
};
//# sourceMappingURL=three_addons_tsl_display_SMAANode__js.js.map
