/**
 * Sepia Tone Post-FX Demo
 * Vintage sepia color grading effect
 */

import * as THREE from 'three/webgpu';
import { pass } from 'three/tsl';
import { sepia } from '@tsl-kit/postfx';
...
export async function init(canvas, controlsContainer) {
  ...
  function updatePostProcessing() {
    ...
    const finalPass = params.enabled ? sepia(scenePass) : scenePass;
    
    postProcessing.outputNode = finalPass;

    // Update material color
    const col = new THREE.Color(params.objectColor);
  ...
  // GUI
  const sepiaFolder = demo.gui.addFolder('Sepia Settings');
  sepiaFolder.add(params, 'enabled').name('Enable Sepia').onChange(updatePostProcessing);
  sepiaFolder.open();

  const sceneFolder = demo.gui.addFolder('Scene');
  sceneFolder.addColor(params, 'objectColor').name('Object Color').onChange(updatePostProcessing);
