/**
 * Blinn-Phong Specular Demo
 * Optimized specular highlights using halfway vector
 */

import * as THREE from 'three/webgpu';
import { vec3, normalWorld, positionWorld, cameraPosition, normalize, Fn, color, uniform } from 'three/tsl';
import { blinnPhongSpecular } from '@tsl-kit/lighting';
...
export async function init(canvas, controlsContainer) {
  ...
  function updateMaterial() {
    material.colorNode = Fn(() => {
      ...
      const spec = blinnPhongSpecular(viewDir, normal, lightDirection, uniform(params.power));
  ...
  const specFolder = demo.gui.addFolder('Blinn-Phong Specular');
