/**
 * Cave Art Material
 * 
 * Prehistoric cave painting effect
 * 
 * @module materials/caveArt
 */

import { Color } from 'three'
import {
  abs,
  add,
  exp,
  float,
  If,
  mix,
  or,
  positionGeometry,
  pow2,
  sub,
} from 'three/tsl'
import { noise, prepare, TSLFn } from './utils.js'

/**
 * Default cave art parameters
 */
const defaults = {
  $name: 'Cave art',
  scale: 2,
  thinness: 2,
  noise: 0.3,
  color: new Color(0xD34545),
  background: new Color(0xFFF8F0),
  seed: 0,
}

/**
 * Cave art material generator
 * 
 * Creates prehistoric cave painting effect
 * 
 * @param params - Material parameters
 * @param params.scale - Overall scale (default: 2)
 * @param params.thinness - Line thinness (default: 2)
 * @param params.noise - Noise amount (default: 0.3)
 * @param params.color - Paint color (default: red)
 * @param params.background - Cave color (default: beige)
 * @param params.seed - Random seed (default: 0)
 * @returns Color node
 * 
 * @example
 * ```typescript
 * import { caveArt } from '@tslstudio/materials'
 * 
 * material.colorNode = caveArt({
 *   scale: 2.5,
 *   thinness: 2.5,
 *   color: new Color(0xCC3300),
 *   background: new Color(0xFFFFE0)
 * })
 * ```
 */
export const caveArt = TSLFn(([params]: any) => {
  params = prepare([params], defaults)

  const pos = positionGeometry.mul(exp(params.scale)).add(params.seed).toVar()

  const k1 = noise(pos, 4).sin().toVar()
  const k2 = noise(pos.mul(1.5), 4).cos().toVar()

  const thinness = exp(sub(float(3), params.thinness))
  const k = sub(thinness, pow2(abs(add(k1, k2))).mul(20)).toVar()

  If(or(k1.greaterThan(k2), k.lessThan(0)), () => {
    k.assign(0)
  })

  If(k.lessThanEqual(0), () => {
    k.assign(params.noise.mul(pow2(noise(pos.mul(30)))))
  })

  return mix(params.background, params.color, k)
}, defaults)

